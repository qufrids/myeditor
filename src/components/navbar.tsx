...
export function Navbar() {
  ...
  // iOS-compatible scroll lock: position:fixed + top offset
  useEffect(() => {
  ...
  // Close menu and reset all sub-states on route change
  useEffect(() => {
    setMobileOpen(false);
          ...
          {mobileOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed right-0 top-0 z-[70] flex h-full w-[min(85vw,360px)] flex-col lg:hidden bg-white dark:bg-slate-950 border-l border-slate-200/50 dark:border-white/[0.06] shadow-[-16px_0_48px_rgba(0,0,0,0.06)]">
                            ...
                            {mobileServicesOpen && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                                ...
                                <div className="flex flex-col gap-0.5 py-1 pl-3">
                                  {services.map((service) => (
                                    <Link key={service.name} href={service.href} onClick={closeMobileMenu}
                ...
                <div className="border-t border-slate-100 dark:border-white/[0.06] p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
                  <Button variant="luxury" size="lg" className="w-full" asChild>
                    <Link href="/order" onClick={closeMobileMenu}>Order Now</Link>
                  </Button>
                </div>
              </motion.div>
            </>
